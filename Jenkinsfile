      ...
      steps {
        ...
        container('docker') {  
          ...
          timeout(time: 60, unit: 'MINUTES') {
            ...
            sh "docker buildx build -t '$DOCKER_IMAGE':'$DOCKER_TAG' --build-arg 'COMMIT_SHA=\$(git rev-parse --short HEAD) APP_VERSION=$VERSION BUILDKIT_INLINE_CACHE=1' --cache-from '$DOCKER_IMAGE':'$DOCKER_TAG' ."
          }
